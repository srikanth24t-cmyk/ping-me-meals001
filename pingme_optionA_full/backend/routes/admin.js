...
router.get('/orders/:id', async (req, res) => {
  try {
    ...
    const items = await pool.query(`SELECT oi.id, oi.qty, oi.menu_id, m.name, m.price FROM order_items oi LEFT JOIN menu m ON m.id = oi.menu_id WHERE oi.order_id=$1`, [id]);
